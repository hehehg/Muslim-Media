...
public class SketchwareUtil {
    ...
    /**
     * Converts Arabic-Indic digits (٠١٢٣٤٥٦٧٨٩) to plain ASCII digits (0-9).
     * Several places in this app parse numeric text typed by the user
     * (session counts, durations, etc.) and only normalized the digits
     * *after* calling Double.parseDouble, which throws NumberFormatException
     * if the user typed with an Arabic-Indic keyboard. Normalize first, then
     * parse.
     */
    public static String normalizeArabicIndicDigits(String input) {
    ...
    /**
     * Normalizes Arabic-Indic digits and parses the result as a double,
     * returning {@code defaultValue} instead of throwing when the text is
     * empty or not a valid number (e.g. user input that hasn't been
     * validated yet).
     */
    public static double parseDoubleSafe(String input, double defaultValue) {
        try {
            String normalized = normalizeArabicIndicDigits(input).trim();
